...
$(document).ready(function() {
    $(document).on("click", "#info-form-submit", function (event) {
        ...
        $.post('https://alzIdentification/editSubmit', JSON.stringify(CurrentData));
        editing = false;
    });
});
...
function SetIDElements(data) {
    ...
    var name = data.name || "Non défini";
    var age = data.age || "Non défini";
    var sex = data.sex || "Non défini";
    var number_id = data.number_id || "Non défini";
    var rank = data.rank || "Non défini";
    var department = data.department || "unknown";
