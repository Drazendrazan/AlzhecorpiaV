...
AddEventHandler('iens:repair', function()
	if isPedDrivingAVehicle() then
		...
		if IsNearMechanic() then
			...
			notification("~g~Tyrol a regardé de plus près ton véhicule ; il est encore plus neuf qu'à sa sortie d'usine !")
			return
		end
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				notification("~g~" .. repairCfg.fixMessages[fixMessagePos] .. ", maintenant allez voir un mécano.")
				fixMessagePos = fixMessagePos + 1
				if fixMessagePos > repairCfg.fixMessageCount then fixMessagePos = 1 end
			else
				notification("~r~Rien à faire, ton véhicule est HS. Va voir Tyrol.")
			end
		else
		...
		notification("~y~Vous devez être dans le véhicule pour pouvoir le réparer.")
	end
end)

RegisterNetEvent('iens:notAllowed')
AddEventHandler('iens:notAllowed', function()
	notification("~r~Vous ne savez pas réparer de véhicules.")
end)
