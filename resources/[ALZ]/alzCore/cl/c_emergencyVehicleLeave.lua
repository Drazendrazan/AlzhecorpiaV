    ...
    while true do
        ...
        if not notify then
            if IsPedInAnyVehicle(ped, true) then
                ShowNotification("Maintenez F pour sortir du véhicule en laissant le moteur allumé.")
                notify = true
            end
        end
        if RestrictEmer then
            ...
            if IsPedInAnyVehicle(ped, false) and IsControlPressed(2, 75) and not IsEntityDead(ped) then
                ...
                if IsPedInAnyVehicle(ped, false) and IsControlPressed(2, 75) and not IsEntityDead(ped) then
                    SetVehicleEngineOn(veh, true, true, false)
                    ShowNotification("Vous êtes sorti du véhicule en laissant le moteur allumé.")
                    if keepDoorOpen then
                        TaskLeaveVehicle(ped, veh, 256)
                    else
                        TaskLeaveVehicle(ped, veh, 0)
