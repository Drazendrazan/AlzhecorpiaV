            ...
            if #(plyPos - coords) <= 1.5 then
                if not inTrunk then
                    if GetVehicleDoorAngleRatio(vehicle, 5) < 0.9 then
                        DrawText3D(coords.x, coords.y, coords.z, '~g~H~w~ Ouvrir | ~r~E~w~ Se cacher')
                        if IsControlJustReleased(0, 74)then
                            SetCarBootOpen(vehicle)
                        end
                    else
                        DrawText3D(coords.x, coords.y, coords.z, '~g~H~w~ Fermer | ~r~E~w~ Se cacher')
                        if IsControlJustReleased(0, 74) then
...
function getInTrunk(veh)
    ...
    if not DoesVehicleHaveDoor(veh, 6) and DoesVehicleHaveDoor(veh, 5) and IsThisModelACar(model) then
        ...
        while inTrunk do
            ...
            DrawText3D(coords.x, coords.y, coords.z, '~r~F~w~ Sortir')
